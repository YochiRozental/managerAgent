...
**הבא:** (1) נגישות חיצונית — שרת ענן (מחכה לחשבון מוטי). (2) שיחה חופשית למוטי מעל כל המשרד (16). (3) הרחבת הצ'אט — "מה הבא?" אחרי סיום שלב. (4) לכוונן ספי הסלמה אחרי ~שבועיים. **אופטימיזציה:** לצמצם את `getReverseDependencyMap` (סריקה קרה ~32ש').
