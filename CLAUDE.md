...
**הבא:** (1) נגישות חיצונית — שרת ענן (מחכה לחשבון מוטי). (2) תדריך בוקר מלא — להוסיף מכירות/גבייה/החלטות (17). (3) שיחה חופשית למוטי מעל כל המשרד (16). (4) הרחבת הצ'אט — "מה הבא?" אחרי סיום שלב. (5) לכוונן ספי הסלמה אחרי ~שבועיים. **אופטימיזציה:** לצמצם את `getReverseDependencyMap` (סריקה קרה ~32ש').
