...
**חבילות מפתח:** `@anthropic-ai/sdk` (tool-use — ניתוב FAST/SMART דרך `src/ai/models.ts`) · `@mondaydotcomorg/api` (GraphQL raw) · `googleapis` (Calendar + Gmail) · `@whiskeysockets/baileys` (WhatsApp Web, לא רשמי, מספר ייעודי) · `luxon` (Asia/Jerusalem) · `zod` · `@huggingface/transformers` (Whisper STT — מכובה כרגע).

**מודלים — Model Router (`src/ai/models.ts`, המקום היחיד עם שמות מודלים):** `MODEL_FAST` (ברירת מחדל `claude-haiku-4-5-20251001`) ל-tool-use פשוט/קריאת מידע/עדכוני סטטוס; `MODEL_SMART` (ברירת מחדל `claude-sonnet-5`) לניתוח/תכנון/החלטות/בקשות עמומות. `chooseModelForTask()` דטרמיניסטי (בלי קריאת AI): FAST כברירת מחדל, SMART לפי regex מילות-מפתח (נתח/תכנן/השווה/תמליץ/עדיף…), שאלת בקרה־על ממי שיש לו `view:all_work`, הודעה >220 תווים, או שיחה עמוקה (14+ הודעות). fallback יחיד FAST→SMART אם FAST נכשל *ולא* בוצעו תופעות לוואי (`shouldEscalateToSmart`). `logAiCall()` מלוגג tier/model/tokens/עלות משוערת בלי תוכן/סודות. שני call-sites: `integrations/claude/orchestrator.ts` (WhatsApp) ו-`ops/chat.ts` (חלונית). `npm run test:router`.
...
- **Model Router — ייעול עלויות AI (2026-09-08).** `src/ai/models.ts` — מקום מרכזי יחיד: `MODEL_FAST`/`MODEL_SMART` (env, defaults `claude-haiku-4-5-20251001` / `claude-sonnet-5`), `chooseModelForTask()` דטרמיניסטי, `shouldEscalateToSmart()`, `addUsage`/`estimateCostUsd`/`logAiCall`. שני ה-call-sites (`orchestrator.ts` WhatsApp, `ops/chat.ts` חלונית) עברו: הלולאה הופרדה ל-`runLoop(model)`, ניתוב לפני, fallback יחיד FAST→SMART (רק אם FAST נכשל ואף כלי/כתיבה לא רץ — מונע הכפלת פעולות ו-retry loops), `logAiCall` אחרי כל ניסיון (tier/model/route_reason/tokens/est_cost_usd — בלי תוכן/מפתחות). כל שמות המודלים הקשיחים (`const MODEL = "claude-sonnet-5"` ×2) הוסרו. **FAST:** ברירת מחדל — שאלות עובד, "בוקר טוב", עדכוני סטטוס, mark done, find task, tool-use פשוט. **SMART:** מילות מפתח ניתוח/תכנון/השוואה/המלצה, שאלת בקרה־על ממי שיש לו `view:all_work` (מוטי/יוכי — "מה תקוע במשרד"), הודעה >220 תווים, שיחה 14+ הודעות, או fallback. `npm run test:router` (ניתוב פשוט/מורכב, env override, חישוב עלות, guard ה-fallback). נבדק חי: הניתוב, ה-logging וה-fallback עבדו (גם דרך שגיאות API — חשבון Anthropic היה בלי קרדיט בזמן הבדיקה, לא קשור לשינוי). **אין קריאות AI מחוץ ל-router.**
