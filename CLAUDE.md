...
- **היסטוריית שיחות בשרת (2026-09-06).** טבלת `chat_messages` (SQLite) + `src/db/repositories/chatHistory.ts`. `POST /api/chat` שומר כל זוג הודעה+תשובה (`appendChatTurn`). `GET /api/chat/history` (120 אחרונות), `POST /api/chat/new` (מפריד `break` — ההיסטוריה נשמרת, רק ההקשר של המודל מתאפס), `POST /api/chat/clear`. ה-UI נגמל מ-localStorage: `state.thread` נטען מהשרת בכל פתיחה, `activeContext()` = ההודעות שאחרי המפריד האחרון (עד 20) — זה מה שנשלח למודל. "שיחה חדשה" מוסיף מפריד ומציג קו "שיחה חדשה". נבדק בדפדפן: היסטוריה שורדת רענון, המפריד מוצג.
