...
**הבא:** (1) נגישות חיצונית — שרת ענן (מחכה לחשבון מוטי). (2) הרחבת הצ'אט — "מה הבא?" אחרי סיום שלב, יצירת משימות המשך. (3) לכוונן ספי הסלמה אחרי ~שבועיים הרצה. **אופטימיזציה:** לצמצם את `getReverseDependencyMap` (סריקה קרה ~32ש').
