...
- מעבר ל-GPT: `AI_FAST_PROVIDER=openai` + `AI_FAST_MODEL=gpt-5.4-mini` (model ID רשמי — reasoning model, תומך function calling ב-Chat Completions) + `OPENAI_API_KEY` — בלי קוד. `npm run test:router` · `npm run test:ai-providers` · `npm run test:ai-compare` (השוואת GPT-5.4 mini מול Claude Haiku, READ-ONLY).
