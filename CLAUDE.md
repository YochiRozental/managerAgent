...
עודכן: 2026-09-10. הפיתוח אצל יוכי (`YochiRozental`). מוטי — מוצר/החלטות/אישורים.
...
**חבילות מפתח:** `@anthropic-ai/sdk` + `openai` (מאחורי שכבת provider אחת) · `@mondaydotcomorg/api` (GraphQL raw) · `googleapis` (Calendar + Gmail) · `@whiskeysockets/baileys` (WhatsApp Web, לא רשמי, מספר ייעודי) · `luxon` (Asia/Jerusalem) · `zod` · `@huggingface/transformers` (Whisper STT — מכובה כרגע).

**שכבת AI — `src/ai/` (כל קריאות ה-AI עוברות כאן, אין `messages.create` בשום מקום אחר):**
- `providers/` — `types.ts` (טיפוסים ניטרליים: NormMessage/NormTool/NormToolCall/RunModelResult), `anthropic.ts` + `openai.ts` (adapters — Chat Completions + function calling), `index.ts` (`runModel({provider,model,...})`). שאר המערכת לא יודעת אם רץ Claude או GPT.
- `tierConfig.ts` — `resolveAiConfig(env)` ממפה FAST/SMART → `{provider, model}` לפי `AI_FAST_PROVIDER`/`AI_FAST_MODEL`/`AI_SMART_PROVIDER`/`AI_SMART_MODEL`. **ברירת מחדל: anthropic (haiku-4-5 / sonnet-5) — בלי env לא משתנה כלום.** `MODEL_FAST`/`MODEL_SMART` = alias ישן. provider לא-תקין → שגיאה ברורה.
- `pricing.ts` — `MODEL_PRICING` (מחירון מרכזי, list price **אומת מול התיעוד הרשמי 2026-09-08**: Anthropic + OpenAI כולל `gpt-5.4-mini` $0.75/$4.50; Sonnet 5 תוקן ל-$2/$10, Opus 5 ל-$5/$25; מודל לא במפה → cost=null, בלי הערכות) + `estimateCostUsd`.
- `models.ts` — ה-**router**: `chooseModelForTask()` דטרמיניסטי בוחר **tier בלבד** (fast/smart) לפי regex מילות-מפתח / שאלת בקרה־על (`view:all_work`) / אורך / עומק שיחה. `shouldEscalateToSmart()`, `logAiCall()` (tier/provider/model/tokens/cost — בלי תוכן/סודות).
- `agentLoop.ts` — לולאת tool-use משותפת provider-agnostic (הרצת כלים דרך callback; `screenToolCalls` לזרימת האישור). `_runModel` injectable לבדיקות.
- `routedAgent.ts` — `runRoutedAgent()` = router → tierConfig → agentLoop → fallback יחיד FAST→SMART (רק אם FAST נכשל ואפס תופעות לוואי — מונע הכפלת פעולות; אין retry loop) → logAiCall. שני ה-call-sites (`integrations/claude/orchestrator.ts`, `ops/chat.ts`) קוראים לזה עם `buildLoop(tier)` בלבד.
- מעבר ל-GPT: `AI_FAST_PROVIDER=openai` + `AI_FAST_MODEL=gpt-5.4-mini` (model ID רשמי — reasoning model, תומך function calling ב-Chat Completions) + `OPENAI_API_KEY` — בלי קוד. `npm run test:router` · `npm run test:ai-providers` · `npm run test:ai-compare` (השוואת GPT-5.4 mini מול Claude Haiku, **READ-ONLY בכל מצב**: `--tools monday|google|all`; `monday` לא טוען את מודולי Google כלל — import דינמי מותנה, אין OAuth; `--list-tools` / `--help` לא מריצים מודלים).
...
**דגלים חשובים בקוד הקיים:** הכלי `list_my_work` בנוי מעל `getMyWorkBrief` (`src/ops/myWorkBrief.ts`) → `getEmployeeDashboard(user)` → `fetchUserOpsTasks` + `buildDashboardViews`. מסונן לפי **המשתמש ששאל** (`ctx.user.mondayUserId`, מוזרק מ-`ToolDefinition.execute(input, ctx)`), לא לפי בעל הטוקן. בחירה: **כל** myDay + כל needsAttention שאינו כבר ב-myDay + עד 3 מ-waitingOnMe שהם critical/stuck/blocking בלבד (השאר רק ב-`summary`). hard cap 30. בלי mondayUserId → שגיאה, בלי fallback. **לא תלוי ב-`MONDAY_BOARD_ID`.**
...
env keys: `ANTHROPIC_API_KEY`, `OPENAI_API_KEY` (רק אם provider=openai), `AI_FAST_PROVIDER`/`AI_FAST_MODEL`/`AI_SMART_PROVIDER`/`AI_SMART_MODEL`/`AI_COST_LOGGING` (שכבת AI — `src/ai/tierConfig.ts`, defaults בטוחים = anthropic, לא חובה; `MODEL_FAST`/`MODEL_SMART` alias ישן), `MONDAY_API_TOKEN`, `MONDAY_BOARD_ID`, `ALLOWED_WHATSAPP_JIDS`, `USER_EMAIL`, `TIMEZONE`, `ENABLE_VOICE_TRANSCRIPTION`, `PORT` (חלונית, 3001), `SESSION_SECRET` (עוגיית הפעלה), `ACCESS_SECRET` (חתימת קישורי כניסה אישיים), `PUBLIC_URL` (כתובת חיצונית לבניית הקישורים), `REQUIRE_ACCESS_LINK` (true = כניסה רק דרך קישור). **חדש (2026-09-10):** `BACKUP_HOUR`/`CONTROL_SCAN_HOUR`/`WEEKLY_REPORT_HOUR` (מתזמן), `GOOGLE_AUTH_INTERACTIVE=true` (רק מקומית, ל-`npm run google-auth`), `OAUTH2_PROXY_CLIENT_ID`/`_CLIENT_SECRET`/`_COOKIE_SECRET`/`_REDIRECT_URL` (profile `auth` ב-compose). הרשימה המלאה + הסברים ב-`.env.example`. כל הסודות כבר ב-`.env` המקומי ובשרת (ערכים אקראיים נוצרו 2026-09-03).
...
- **מחיקת שיחה מההיסטוריה (2026-09-06).** `deleteSession(userKey, sessionId)` ב-`chatHistory.ts` + `POST /api/chat/session/delete {id}`. כל שורה ב-overlay של "היסטוריית שיחות" קיבלה כפתור "מחק" (`.hist-del`) לצד אזור הטעינה (`.hist-main`); `confirm()` לפני, ואם מוחקים את השיחה הפעילה — המסך מתאפס כמו "שיחה חדשה". `renderHistory()` הופרד מ-`#historyBtn.onclick` כדי לרענן את הרשימה אחרי מחיקה. נבדק: מחיקה דרך ה-endpoint מורידה את השיחה, הרשימה מתרעננת ל"אין שיחות שמורות עדיין".
...
- **Model Router — ייעול עלויות AI (2026-09-08).** מנתב FAST(Haiku)/SMART(Sonnet) דטרמיניסטית לפי סוג הבקשה, עם fallback יחיד. הוחלף/הורחב ב-8 בספט' לשכבת provider (ראה למטה) — הפרטים ב-`src/ai/`.

- **שכבת AI Provider — Anthropic + OpenAI (2026-09-08).** בקשת יוכי: לבחור provider+model לכל tier דרך env, בלי קוד. נבנה `src/ai/providers/` (`types.ts` פורמט ניטרלי · `anthropic.ts`/`openai.ts` adapters — OpenAI דרך Chat Completions + function calling · `index.ts` `runModel()`), `tierConfig.ts` (`resolveAiConfig` — `AI_FAST_PROVIDER`/`AI_FAST_MODEL`/`AI_SMART_PROVIDER`/`AI_SMART_MODEL`, ברירת מחדל anthropic, alias `MODEL_FAST`/`MODEL_SMART`, provider לא-תקין → throw), `pricing.ts` (`MODEL_PRICING` מרכזי + `estimateCostUsd`), `agentLoop.ts` (לולאת tool-use משותפת provider-agnostic, `_runModel` injectable), `routedAgent.ts` (`runRoutedAgent` = router→tierConfig→loop→fallback→log). `models.ts` צומצם ל-router בלבד (`chooseModelForTask` מחזיר `{tier, reason}` — לא יודע על provider/model). **שני ה-call-sites** (`orchestrator.ts`, `ops/chat.ts`) הצטמצמו — כל אחד רק `buildLoop(tier)` + פירוש התוצאה; זרימת האישור ב-orchestrator עברה ל-`screenToolCalls`; הגשת תדריך הבוקר מילה-במילה ב-`finalizeText`. `logAiCall` קיבל `provider`. הותקן `openai@^7`. `env.ts` קיבל `OPENAI_API_KEY` (optional). **התנהגות ברירת מחדל לא השתנתה** (anthropic haiku/sonnet). בדיקות: `npm run test:ai-providers` (13 תרחישים: FAST/SMART × anthropic/openai, fallback חוצה-ספקים בשני הכיוונים, env override, provider לא-תקין, תרגום schema/messages/response, אין tool-exec כפול, אין קריאת AI כפולה) + `test:ai-compare` (השוואת ספקים ידנית, READ-ONLY: בלי כלים כברירת מחדל; `--tools monday|google|all`, `monday` בלי טעינת Google/OAuth דרך import דינמי מותנה; `--list-tools`/`--help` בלי API). **נבדק חי:** `runOpsChat` מקצה-לקצה — Monday נמשך, ניתוב FAST→fallback SMART, `logAiCall` בפורמט החדש (provider+tier+tokens), החזרה graceful (Anthropic עדיין בלי קרדיט — לא קשור). **אין קריאות AI מחוץ לשכבה.** המחירון + ה-model IDs של OpenAI אומתו מול התיעוד הרשמי (gpt-5.4-mini). production env לא שונה — עדיין anthropic; המעבר ל-OpenAI מתוכנן אחרי פריסה + הוספת מפתח מאובטח + השוואה אמיתית.

- **`list_my_work` — חיבור לתשתית הדשבורד (2026-09-09).** חקירה גילתה שהמימוש הישן (`listMyWork` על `MONDAY_BOARD_ID` + `assigned_to_me`) החזיר 447 פריטים גולמיים של **בעל טוקן ה-API** (לא המשתמש ששאל), ~74KB, בלי done-filter תקין (זיהה עמודת "סוג משימה" כסטטוס), בלי סינון תאריך. **תיקון:** `src/ops/myWorkBrief.ts` — `getMyWorkBrief(user)` → `getEmployeeDashboard(user)` (reuse מלא, `getEmployeeDashboard` לא שונה) → `buildBrief` (טהור): כל myDay + needsAttention לא-כפול + עד 3 מ-waitingOnMe critical/stuck/blocking בלבד, cap 30. שדות שימושיים בלבד (project/stage/status/priority/dueDate/daysOverdue/dueToday/stuck/critical/blockingOthers/ballWith/url) + `summary` עם הספירות המלאות. `ToolDefinition.execute` קיבל `ctx: ToolContext { user }` — ה-orchestrator ו-`messageHandler` מזריקים את המשתמש. `listMyWork`/`MyWorkItem` הוסרו מ-`tasks.ts`. `test-ai-compare` קיבל `--user <key>` / `--monday-user-id <id>`, ובלי אחד מהם `list_my_work` מחזיר שגיאה. `npm run test:mywork`. **נבדק חי:** דוב 84 raw → 6 myDay / 5 needsAttention / 16 waitingOnMe → **9 brief** (6 today + 3 waiting-extras), 3.5KB. Haiku input 38,868→~5,600 tokens, Sonnet 27,061→~4,700. **edge case פתוח:** משימות overdue ישנות מאוד (430 ימים) עדיין ב-myDay — התנהגות קיימת של `buildDashboardViews`, משותפת עם החלונית; לשנות = החלטת מוצר.

- **פריסה לשרת Hetzner (2026-09-10).** הסוכן חי עכשיו בפרודקשן על `gotlib-production` (`167.233.150.99`, CPX12, Ubuntu 24.04). `docker compose` → `whatsapp-agent` + `window` (127.0.0.1:3001), Caddy על ה-host → `manager-agent.gotlib.biz` + TLS. `restart: unless-stopped` + Docker enabled = שורד reboot. הקשחה: SSH key-only, ufw (22/80/443), fail2ban, swap 2GB, `mem_limit`. גישה: `ssh -i ~/.ssh/gottlieb_agent root@167.233.150.99`. עדכון: `cd /opt/managerAgent && git pull && docker compose build && docker compose up -d`. `docs/deployment-hetzner.md`. **פתוח:** WhatsApp צריך pairing (מסך QR); `data/google-token.json` חסר → Calendar/Gmail כבויים (הסבב היומי מתפקד בלעדיהם — `GOOGLE_AUTH_INTERACTIVE=true` + `npm run google-auth`); oauth2-proxy (profile `auth`) מחכה ל-OAuth client מ-Google; production `.env` מריץ `AI_FAST_PROVIDER=openai`/`gpt-5.4-mini`.

- **יציבות ברקע (2026-09-10).** `GET /health` (ציבורי, 200/503, סיבות בעברית) · טבלאות `heartbeats` + `job_runs` + `src/db/repositories/systemHealth.ts` · `src/ops/backup.ts` (`VACUUM INTO data/backups/`, 14 יום, מהמתזמן) · `src/ops/scheduler.ts` שוכתב עם catch-up אחרי ריסטרט (מריץ עבודה שהוחמצה, ומריץ שוב ריצה תקועה >30 דק') · `src/db/db.ts` WAL + busy_timeout · `server/index.ts` crash handlers שלא מפילים על תקלת Monday/AI זמנית (DEGRADED + `noteIncident`) · `npm run test:health`.

- **הלולאה: פנייה יזומה + סגירה מול Monday (2026-09-10, מטרות הבקשה של יוכי).** מנוע הבקרה פונה לעובד בחלון שלו ומטפל בתשובה חופשית, וממשיך לעקוב.
  - **`finding_events`** (טבלה + `src/db/repositories/findingEvents.ts`): `nudge_sent` / `employee_responded` / `snoozed` / `resolved_by_reply` / `manager_pinged`. `control_findings` + `Finding` נושאים `item_id`/`item_source` (המשימה שהממצא נוגע לה).
  - **`escalationDecision(finding, now)`** (מופרד + נבדק ב-`test:loop`): snoozed לעתיד → שקט עד התאריך · תגובת עובד → שעון ה-staleness מתאפס · `resolved_by_reply` → דלג. **תוקן `businessDaysBetween`** — השרת רץ ב-UTC וה-first_seen נשמר עם +03:00, אז ההשוואה חתכה יום → נרמול ל-`env.TIMEZONE`. (השפעה: כל הממצאים "מתבגרים" יום מוקדם יותר — נכון לפי הספק.)
  - **פנייה יזומה:** הסלמה רמה 1 שולחת נודג' קונקרטי ("Yochi, המשימה X — באיחור N ימים. מה המצב?"), נשמר כ-notification עם הקשר משימה + נדחף חי דרך SSE (`src/ops/nudgeBus.ts`). `GET /api/nudges` + `GET /api/events`. ה-UI מרנדר נודג'ים כבועות עוזר, טוסט בהגעה חיה, catch-up בפתיחה.
  - **סגירת הלולאה:** `runOpsChat(user, history, { about: {itemId, source, findingKey} })`. 7 כלי `reply_*` (`src/ops/loopReply.ts`) ממפים 8 כוונות → פעולת Monday נכונה בהרשאות + Update מתעד + `finding_event`. "מחכה למנהל" → notification למוטי. תשובה-לנודג' נכפית ל-SMART (דיוק > עלות). זהות: `TeamMember.aliases` — "Yochi" (שם ב-Monday) נפתר ליוכי.
  - **נבדק E2E על השרת** (`scripts/e2e-loop.ts`, `E2E_KEEP`/`E2E_CLEANUP_ONLY`): משימת בדיקה אמיתית → סריקה → נודג' → תשובה "צריך עוד יומיים, מחכה לקונסטרוקטור" → הסוכן (Sonnet) בחר `reply_defer` → תאריך היעד עודכן, Update נרשם, snooze → סבב לא מסלים בזמן ההשהיה → אחרי שההשהיה נגמרה המעקב חוזר → "סיימתי" → סטטוס "בוצע" + הממצא נסגר. **גם דרך הדפדפן** (הקישור האישי של יוכי): "מחכה ליועץ קונסטרוקטור" → `reply_waiting(consultant)` → Monday עודכן, `employee_responded` נרשם.

**הבא:** (1) WhatsApp pairing + oauth2-proxy (מחכה לפעולה של יוכי). (2) `data/google-token.json` להחזרת Calendar/Gmail. (3) לכוונן ספי הסלמה אחרי ~שבועיים. (4) מטרות פתוחות: 11 מכרזים · 19 זיכרון מובנה · 20 דפוסים. **אופטימיזציה:** `getReverseDependencyMap` (סריקה קרה ~32ש').
