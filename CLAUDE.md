...
**הבא:** (1) שלב 2ב — דיווח שעות מהחלונית: כניסה/יציאה → שעון הנוכחות פר-עובד (בורד `1550734548` → `board_relation__1`), טיימר פר-פרויקט → עמודות `time_tracking*` בבורד `1550734533`. (2) שלב 0 — שרת ענן — מחכה לחשבון + אישור מוטי.
