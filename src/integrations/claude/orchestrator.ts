...
export async function runOrchestrator(history: ConversationMessage[]): Promise<OrchestratorResult> {
  ...
  for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
    ...
    for (const toolUse of toolUses) {
      ...
      if (!tool) {
        ...
        try {
          logger.info({ tool: toolUse.name, input: toolUse.input }, "מריץ כלי");
