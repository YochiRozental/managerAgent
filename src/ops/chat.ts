...
export const CREATE_TASK_TOOL_DECL = {
  name: "create_task",
  description:
    "פותח משימה חדשה ב-Monday. שלושה סוגי יצירה: (1) בלי project — משימה כללית בלוח המשימות, בלי שום קישור, נוצרת מיד. (2) עם project ו-taskKind='project' (או שאין stage מפורש והוזכר 'תחת הפרויקט') — item באותו לוח, אבל *עם* קישור אמיתי לפרויקט. (3) עם project ו-stage מפורש (או taskKind='stage') — subitem תחת השלב. כש-project ניתן צריך גם stage מפורש וגם taskKind כדי לדעת אם מדובר ב-(2) או ב-(3); בלי אף אחד מהשניים הכלי יזרוק שגיאה עם השאלה שצריך לשאול את המשתמש — זה תקין ומצופה, לא כשל. חובה: אם ניתן project, הוא חייב להינתן *בכל קריאה* שעוסקת באותה משימה, כולל קריאות המשך אחרי ששאלת taskKind/stage — אחרת המשימה עלולה להיווצר בלי הקישור לפרויקט. בלי assignee — מוקצית למשתמש עצמו. חובה שיהיה taskName אמיתי לפני הקריאה — אל תקרא לכלי הזה כדי 'לבדוק' מה קורה בלי תוכן משימה אמיתי.",
  input_schema: {
    type: "object",
    properties: {
      taskName: {
      ...
      taskKind: {
        ...
        enum: ["project", "stage"],
        description:
          "רק כש-project ניתן וגם stage לא ניתן מפורש. 'project' = item בלוח המשימות הכלליות *עם* קישור לפרויקט (לא subitem, לא שלב). 'stage' = subitem תחת אחד משלבי הפרויקט. אל תמלא לבד — זו החלטה עסקית של המשתמש: אם לא ברור מהניסוח, אל תקרא לכלי, שאל 'להוסיף את המשימה תחת הפרויקט, או תחת אחד משלבי הפרויקט?' וחכה לתשובה. אל תשתמש במילה 'כללית' לתאר את (2) — זה מטשטש בין משימה בלי שום פרויקט לבין משימה עם קישור לפרויקט.",
      },
      stage: {
        type: "string",
        description:
...
export function systemPrompt(user: IdentifiedUser, about?: LoopContext): string {
  ...
  return [
    ...
    ...(canCreateTask(user)
      ? [
          "• 'תפתח משימה...' / 'תוסיף משימה...' / 'תיצור משימה...' — קרא create_task, אבל רק אחרי ששני דברים ברורים: (א) taskName אמיתי, (ב) אם ניתן פרויקט — גם אם זו משימה כללית או תחת שלב. סדר השאלות לא קריטי (אפשר לשאול שניהם בבת אחת אם שניהם חסרים), אבל אסור ליצור לפני ששניהם הובהרו.",
          ...
          "• יש שלושה סוגי משימה: (1) בלי פרויקט בכלל — משימה כללית, נוצרת ישר, בלי שאלה. (2) עם פרויקט, תחת הפרויקט עצמו (לא שלב) — item עם קישור לפרויקט. (3) עם פרויקט, תחת אחד משלביו — subitem. אל תשתמש במילה 'כללית' לתיאור (2) — זה מטשטש בין (1) ל-(2) וגורם לבלבול.",
          "• אם ניתן שם פרויקט: זו **החלטה עסקית של המשתמש, אסור לך לבחור לבד** בין (2) ל-(3). אם המשתמש כבר ציין שלב מפורש ('בשלב 4', 'בשלב הרישוי') — זה מכריע לבד, תעביר את זה כ-stage ואל תשאל כלום. אם המשתמש אמר במפורש 'תחת הפרויקט'/'לא בשלב מסוים'/'תקשר לפרויקט בלבד' וכדומה — תעביר taskKind='project' ואל תשאל כלום. אחרת (ניתן פרויקט, אין שלב מפורש, אין ניסוח מפורש) — אל תקרא לכלי, שאל: 'האם להוסיף את המשימה תחת הפרויקט, או תחת אחד משלבי הפרויקט?' ותחכה לתשובה.",
          "• כשהמשתמש עונה 'תחת הפרויקט' → קרא עם taskKind='project'. כשהמשתמש עונה 'תחת שלב' בלי לנקוב באיזה → קרא עם taskKind='stage' בלי stage; הכלי יחזיר שגיאה עם רשימת השלבים האמיתיים של הפרויקט — זה תקין, הצג את הרשימה בדיוק כפי שהתקבלה (כולל המספור 'שלב N') ושאל איזה שלב, ואז קרא שוב עם ה-stage שנבחר.",
          "• **קריטי: project הוא לא 'פעם אחת וזהו'.** בכל קריאה חוזרת ל-create_task על אותה משימה — כולל הקריאה אחרי ששאלת 'תחת הפרויקט או תחת שלב' וקיבלת תשובה — חובה להעביר שוב את project (ואת taskName ואת assignee אם ניתנו), גם אם כבר הועברו בקריאה קודמת. כל קריאה עצמאית: אם לא תעביר project בקריאה שבה אתה כן מעביר taskKind, המשימה תיווצר מנותקת מהפרויקט בלי שתדע — זה קרה בפועל וגרם לתקלה. שמור context בין הודעות כדי לדעת *מה* לשלוח שוב, אבל תמיד שלח את זה מפורשות בכל קריאה.",
          "• אם ציינו למי ('לדוב', 'לרוחמה') — העבר את זה כ-assignee; בלי זה המשימה תיפתח על שם המשתמש עצמו. אם הכלי מחזיר שגיאה (פרויקט/שלב/עובד לא נמצא או עמום, או שאלת תחת-הפרויקט/תחת-שלב) — הצג את האפשרויות/השאלה ושאל, אל תנחש. יש לך כלי יצירה אמיתי — לעולם אל תגיד שאין לך אפשרות ליצור משימה.",
        ]
      : []),
    ...(canCreateLead(user)
      ? [
