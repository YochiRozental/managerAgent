...
export const CREATE_TASK_TOOL_DECL = {
  name: "create_task",
  description:
    "פותח משימה חדשה ב-Monday, במקום הנכון במבנה פרויקט→שלב→משימה. בלי פרויקט — משימת משרד כללית, נוצרת מיד. עם פרויקט — צריך גם לדעת אם זו משימה כללית המקושרת לפרויקט או subitem תחת שלב: אם ניתן stage מפורש הוא מכריע; אחרת צריך taskKind ('general' או 'stage'); בלי אף אחד מהשניים הכלי יזרוק שגיאה עם השאלה שצריך לשאול את המשתמש — זה תקין ומצופה, לא כשל. בלי assignee — מוקצית למשתמש עצמו. חובה שיהיה taskName אמיתי לפני הקריאה — אל תקרא לכלי הזה כדי 'לבדוק' מה קורה בלי תוכן משימה אמיתי.",
  input_schema: {
    type: "object",
    properties: {
      taskName: {
      ...
      taskKind: {
        ...
        enum: ["general", "stage"],
        description:
          "רק כש-project ניתן וגם stage לא ניתן מפורש. 'general' = משימה כללית המקושרת לפרויקט (בלי שלב). 'stage' = תחת אחד משלבי הפרויקט. אל תמלא לבד — זו החלטה עסקית של המשתמש: אם לא ברור מהניסוח, אל תקרא לכלי, שאל 'להוסיף כמשימה כללית המקושרת לפרויקט, או תחת אחד משלבי הפרויקט?' וחכה לתשובה.",
      },
      stage: {
        type: "string",
        description:
  ...
  return [
    ...
    ...(canCreateTask(user)
      ? [
          ...
          "• אם ניתן שם פרויקט: המשימה יכולה להיות (א) משימה כללית המקושרת לפרויקט, או (ב) subitem תחת אחד משלבי הפרויקט — **זו החלטה עסקית של המשתמש, אסור לך לבחור לבד**. אם המשתמש כבר ציין שלב מפורש ('בשלב 4', 'בשלב הרישוי') — זה מכריע לבד, תעביר את זה כ-stage ואל תשאל כלום. אם המשתמש אמר במפורש 'משימה כללית'/'תקשר לפרויקט בלבד' וכדומה — תעביר taskKind='general' ואל תשאל כלום. אחרת (ניתן פרויקט, אין שלב מפורש, אין ניסוח 'כללית') — אל תקרא לכלי, שאל: 'להוסיף את המשימה כמשימה כללית המקושרת לפרויקט, או תחת אחד משלבי הפרויקט?' ותחכה לתשובה. כשהמשתמש עונה 'כללית' → קרא עם taskKind='general'. כשהמשתמש עונה 'תחת שלב' בלי לנקוב באיזה — קרא עם taskKind='stage' בלי stage; הכלי יחזיר שגיאה עם רשימת השלבים האמיתיים של הפרויקט — זה תקין, הצג את הרשימה בדיוק כפי שהתקבלה (כולל המספור 'שלב N') כדי שהמשתמש יוכל להתייחס אליה בבירור, ושאל איזה שלב, ואז קרא שוב עם ה-stage שנבחר. בלי פרויקט בכלל — משימת משרד כללית, נוצרת ישר, בלי השאלה הזו.",
          "• שמור context בין הודעות: אם פרויקט/מבצע/taskName כבר נמסרו בהודעה קודמת באותה שיחה — אל תשאל עליהם שוב, רק על מה שעדיין חסר (למשל רק כללית/שלב).",
          "• אם ציינו למי ('לדוב', 'לרוחמה') — העבר את זה כ-assignee; בלי זה המשימה תיפתח על שם המשתמש עצמו. אם הכלי מחזיר שגיאה (פרויקט/שלב/עובד לא נמצא או עמום, או שאלת כללית/שלב) — הצג את האפשרויות/השאלה ושאל, אל תנחש. יש לך כלי יצירה אמיתי — לעולם אל תגיד שאין לך אפשרות ליצור משימה.",
        ]
      : []),
    ...(canCreateLead(user)
      ? [
...
export async function runOpsChat(
  ...
  if (canCreateTask(user)) {
    tools.push({
      ...
      run: async (input) => {
        const taskKind = input.taskKind === "general" || input.taskKind === "stage" ? input.taskKind : undefined;
